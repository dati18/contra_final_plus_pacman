...
pusher::pusher(DOUBLE2 pos) : ObjectBase(pos), m_AnimationTick(0), m_PointingDir(), m_TimeToDie(30)
{
	m_State = STATE_NORMAL;
	m_Type = TYPE_PUSHER;

	if (m_BmpSpritePtr == 0) { // Bmp nog niet ingeladen
		m_BmpSpritePtr = new Bitmap("./resources/ghost.gif");
...
pusher::~pusher() { // dont delete the static Bmp
...
void pusher::CollideWith(ObjectBase *colliderptr) {
...
void pusher::SetPlayerPtr(ObjectBase *playerPtr) {
...
void pusher::Tick(double deltaTime)
{
	if (m_State == STATE_DIE) {
		m_Pos += m_Velocity * deltaTime;
		this->m_BmpSpritePtr = new Bitmap("./resources/blueghost.gif");
		m_State = STATE_NORMAL;
		return;
	}
...
void pusher::DoCollitions(HitRegion **hitHoriPtr, HitRegion **hitVertPtr, DOUBLE2 *pos, DOUBLE2 *velocity)
{
...
double pusher::ReturnGoodLen(double getal1, double getal2, double centerXY)
{
	double delta = 0;
