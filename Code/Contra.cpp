...
void Contra::GameStart()
{
	...
	if (m_LetsMakeSound) {
		m_MusicPtr = new Audio(IDR_MUSIC, "MP3"); // heavy
		m_MusicPtr->SetVolume(20); // not working
		m_MusicPtr->Play();
		m_MusicPtr->GetRepeat();
	}
	//m_BmpBackgroundPtr = new Bitmap("resources/hqdefault.jpg");
	...
	m_HitTerrainPtr->CreateFromFile("resources/bgHit.svg");

	m_ObjectListPtr = new ObjectList(m_LevelMax, &m_MatView, m_HitTerrainPtr);


	PlaceEnemies(m_ObjectListPtr); // Put all enemies on the correct places
}

void Contra::GameEnd()
{
	// Insert the code that needs to be executed at the closing of the game
	delete m_ObjectListPtr; // Also deletes the objects from the list
	delete m_MusicPtr;
...
void Contra::GameTick(double deltaTime)
{
	...
	if (GAME_ENGINE->IsKeyDown('R')) {
		...
		PlaceEnemies(m_ObjectListPtr); // Spawnt alle enemys in het level.
...
void Contra::GamePaint(RECT rect)
{
	...
	*/
	...
	//GAME_ENGINE->FillRect(-INFINITE, m_GameSize, INFINITE, INFINITE); // Down
	GAME_ENGINE->FillRect(m_GameSize, 0, INFINITE, m_GameSize); // Right
...
void Contra::PlaceEnemies(ObjectList * objectListPtr) {

	// Could use XML save file, but this is good enough


	m_PlayerPtr = new Player(120, 50);
	...
	HiddenSniper *hiddenSniperPtr; // BOSJESSNIPERS---------------------

	hiddenSniperPtr = new HiddenSniper(DOUBLE2(1360, 80));
	hiddenSniperPtr->SetPlayerPtr(m_PlayerPtr);
	objectListPtr->Add(hiddenSniperPtr);

	hiddenSniperPtr = new HiddenSniper(DOUBLE2(1555, 55));
	hiddenSniperPtr->SetPlayerPtr(m_PlayerPtr);
	objectListPtr->Add(hiddenSniperPtr);
	...
	pusher *pusherPtr1;
	pusherPtr1 = new pusher(DOUBLE2(100, 10));
	pusherPtr1->SetPlayerPtr(m_PlayerPtr);
	objectListPtr->Add(pusherPtr1);

	pusher *pusherPtr2;
	pusherPtr2 = new pusher(DOUBLE2(400, 10));
	pusherPtr2->SetPlayerPtr(m_PlayerPtr);
	objectListPtr->Add(pusherPtr2);

	pusher *pusherPtr3;
	pusherPtr3 = new pusher(DOUBLE2(600, 10));
	pusherPtr3->SetPlayerPtr(m_PlayerPtr);
	objectListPtr->Add(pusherPtr3);

	pusher *pusherPtr4;
	pusherPtr4 = new pusher(DOUBLE2(800, 10));
	pusherPtr4->SetPlayerPtr(m_PlayerPtr);
	objectListPtr->Add(pusherPtr4);
}
